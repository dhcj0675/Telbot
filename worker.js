...
async function buildUsersCSV(env){
  ...
  for (const v of vals){ if(!v) continue; let o; try{o=JSON.parse(v)}catch{continue}
...
async function buildPhonesCSV(env){
  ...
  for (const {name} of l.keys){
    ...
    const ujson = await env.KV.get(`user:${id}`); let u={}; try{u=JSON.parse(ujson||"{}")}catch{}
...
async function notifyAdmins(env, from, text, tag=""){
  ...
  for (const adminId of ADMINS){
    try{ await send(env, adminId, header + text); }catch(e){ console.error("notify admin failed:", adminId, e); }
  }
}
...
export default {
  async fetch(request, env) {
    ...
    if (request.method==="POST" && url.pathname===`/webhook/${env.WH_SECRET}`) {
      ...
      // Commands
      let me = { result: { username: "" } }; try{ me = await tg(env,"getMe",{});}catch{}
      ...
      // ForceReply to admin (silent to user)
