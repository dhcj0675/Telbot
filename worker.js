// worker.js — Telegram bot on Cloudflare Workers (Fast ACK + KV + CSV)
// -----------------------------------------------

// آیدی عددی ادمین‌ها
const ADMINS = [6803856798];

// --- Anti-flood (ریت‌لیمیت) ---
const RATE_LIMIT = 8;   // 8 پیام
const WINDOW_TTL = 10;  // در 10 ثانیه
const BLOCK_TTL  = 10;  // بلاک 10 ثانیه
...
async function rateLimitExceeded(env, userId) {
  if (!env.KV) return false;      // اگر KV وصل نباشد، محدودیتی اعمال نمی‌شود
  ...
  try {
    ...
    if (update?.callback_query) {
      ...
      if (data === "csv_users") {
        ...
        const csv = await buildPhonesCSV(env); await sendCSVDocument(env, chatId, "phones.csv", "CSV شماره‌ها");
      } else if (data === "prod_1") {
    ...
    // دریافت شماره (اکنون با پیام تأیید برای کاربر)
    ...
    // اگر کاربر روی پیام ادمین Reply کرد (اکنون با تأیید به کاربر)
    ...
    if (repliedText && (repliedText.includes("##ADMIN##") || repliedText.includes("##ADMIN:"))) {
      ...
      await send(env, chatId, "پیام‌تون برای ادمین ارسال شد ✅", { reply_markup: kb }); // ✅
      return;
    }
